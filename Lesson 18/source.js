...
function RenderScene()
{
    ...
    for(var i = 0; i < CubeFaces.length; i++)
    {
        ...
        var Angle = LightVector.x * OutVector.x + LightVector.y * OutVector.y + LightVector.z * OutVector.z;
        Angle = Math.acos(Angle) / Math.PI;
        
        // Change color based on factor
        Color.R = Math.floor(255.0 * Angle);
        Color.G = Math.floor(255.0 * Angle);
        Color.B = Math.floor(255.0 * Angle);
